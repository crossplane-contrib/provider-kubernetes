...
func main() {
	var (
		...
		enableSanitizeSecrets    = app.Flag("enable-sanitize-secrets", "when enabled, redacts Secret data from Object status").Default("false").Envar("ENABLE_SANITIZE_SECRETS").Bool()
	)
	...
	kingpin.FatalIfError(object.Setup(mgr, o, *enableSanitizeSecrets), "Cannot setup controller")
